function [motion, logger, A, Z] = estSelfMotion(flow, W, f, opt)
% estSelfMotion
...
%       34(21):2917–2938.
%   Our implementation takes the idea about flow templates and applies them 
%   to the 6D space of self-motions rather than assuming a separation of
%   linear and rotational velocity like in Perrone (1992) or a fixating
%   self-motion like in Perrone & Stone (1994). This approach requires a
%   large amount of templates to achieve a dense sampling of possible
%   self-motions. In order to solve this problem we apply a hierarchical
%   grid (HG) search for self-motions assuming a smooth response surface in 
%   the 5D space defined by two translational directions and three 
...
if strcmp(type,'hg')
    [motion, logger, A, Z] = voteSelfMotionHG(flow,W,f,opt);    
else
    [motion, logger, A, Z] = voteSelfMotion(flow,W,f,opt);
end
...
% *************************************************************************
function [motion, logger, A, Z] = voteSelfMotion(flow, W, f, opt)
Dims        = size(flow.X);
MapPhi      = opt.MapPhi;
MapRad      = opt.MapRad;
...
X   = repmat(flow.X(:),  [1 dptNum phiNum radNum oxNum oyNum ozNum]);
Y   = repmat(flow.Y(:),  [1 dptNum phiNum radNum oxNum oyNum ozNum]);
Dx  = repmat(flow.Dx(:), [1 dptNum phiNum radNum oxNum oyNum ozNum]);
Dy  = repmat(flow.Dy(:), [1 dptNum phiNum radNum oxNum oyNum ozNum]);
InvZ = repmat(1./MapDepth,      [vecNum 1 phiNum radNum oxNum oyNum ozNum]);
VelX = shiftdim(repmat(MapVelX, [1 1 oxNum oyNum ozNum vecNum dptNum]), 5);
VelY = shiftdim(repmat(MapVelY, [1 1 oxNum oyNum ozNum vecNum dptNum]), 5);
VelZ = shiftdim(repmat(MapVelZ, [1 1 oxNum oyNum ozNum vecNum dptNum]), 5);
OmegaX = shiftdim(repmat(MapOmegaX, [1 1 1 vecNum dptNum phiNum radNum]), 3);
OmegaY = shiftdim(repmat(MapOmegaY, [1 1 1 vecNum dptNum phiNum radNum]), 3);
OmegaZ = shiftdim(repmat(MapOmegaZ, [1 1 1 vecNum dptNum phiNum radNum]), 3);
if length(W)>1, W = repmat(W, [1 dptNum phiNum radNum oxNum oyNum ozNum]); end
...
A = reshape(A, [vecNum dptNum phiNum radNum oxNum oyNum ozNum]);
% Compute the maximum over all voted depths.
[A, Index] = max(A,[],2);
...
% Determine the depths that lead to a maximum match.
if nargout>3,
    Index   = squeeze(Index);
    Z       = reshape(MapDepth(Index(:,iPhi,iRad,iOx,iOy,iOz)), Dims);
end
...
% (hg) search.
% *************************************************************************
function [motion, logger, A, Z] = voteSelfMotionHG(flow,W,f,opt)
